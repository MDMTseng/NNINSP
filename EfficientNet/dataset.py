...
class PlatingDefectDataset(Dataset):
    ...
    def _initialize_dataset(self):
        """初始化數據集的圖片和標籤"""
        # 獲取所有缺陷類型目錄
        defect_types = [d for d in self.image_base_dir.iterdir() if d.is_dir()]
        ...
        # 創建缺陷類型映射
        unique_defects = sorted(set(self.labels))
        self.defect_to_idx = {defect: idx for idx, defect in enumerate(unique_defects)}
    ...
    def __getitem__(self, idx):
        ...
        if idx < len(self.annotations):
            ...
            for obj in root.findall('.//object'):
                ...
                # print(f"Mask shape: {mask[class_idx].shape}   original_size: {original_size}")
